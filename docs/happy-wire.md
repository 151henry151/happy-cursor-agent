...
This document describes the shared wire package: `@happy-cursor/wire`.
...
`@happy-cursor/wire` centralizes those shared schemas and types so all clients and services agree on the same wire contract.

## Package identity

- npm name: `@happy-cursor/wire`
...
- Session protocol imports now reference `@happy-cursor/wire` directly.
- `src/sessionProtocol/types.ts` now re-exports from `@happy-cursor/wire` as compatibility shim.
- API wire schemas in `src/api/types.ts` now source shared message/update schemas from `@happy-cursor/wire`.

### App (`packages/happy-app`)

- Shared API message/update schemas in `sources/sync/apiTypes.ts` now import these from `@happy-cursor/wire`:
  - `ApiMessageSchema`
...
- Prisma JSON message content type now references `SessionMessageContent` from `@happy-cursor/wire`.
- Event router uses shared `SessionMessageContent` type for `new-message` payload typing.

### Agent (`packages/happy-agent`)

- `RawMessage` now aliases `SessionMessage` from `@happy-cursor/wire`.

## Versioning model

All other workspace packages now declare a versioned dependency on `@happy-cursor/wire`.

This intentionally mirrors post-publish consumption and reduces hidden coupling to workspace-local files.

## Build and release

`@happy-cursor/wire` is configured the same way as existing publishable libraries in this repo:
...
```bash
yarn workspace @happy-cursor/wire release
```

When building workspaces from a clean checkout, build `@happy-cursor/wire` first so dependent packages can resolve generated `dist` outputs.
...
3. Bump and release `@happy-cursor/wire`.
