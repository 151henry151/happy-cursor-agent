...
(async () => {
  ...
  if (subcommand === 'doctor') {
    ...
    // Handle cursor-agent command
    try {
      const { runCursor } = await import('@/cursor/runCursor');

      let startedBy: 'daemon' | 'terminal' | undefined = undefined;
      let startingMode: 'local' | 'remote' | undefined = undefined;
      const cursorPassthroughArgs: string[] = [];
      const cursorEnvVars: Record<string, string> = {};
      let jsRuntimeOverride: string | undefined = undefined;
      ...
      for (let i = 1; i < args.length; i++) {
        ...
        } else if (args[i] === '--cursor-env') {
          const envArg = args[++i];
          if (envArg && envArg.includes('=')) {
            const eqIndex = envArg.indexOf('=');
            cursorEnvVars[envArg.substring(0, eqIndex)] = envArg.substring(eqIndex + 1);
          }
        } else if (args[i] === '--yolo') {
          cursorPassthroughArgs.push('--force');
        } else {
          cursorPassthroughArgs.push(args[i]);
      ...
      const cursorOptions: any = {
        startedBy,
        startingMode,
        cursorEnvVars: Object.keys(cursorEnvVars).length > 0 ? cursorEnvVars : undefined,
        cursorArgs: cursorPassthroughArgs.length > 0 ? cursorPassthroughArgs : undefined,
      };

      await runCursor(credentials, cursorOptions);
    } catch (error) {
      console.error(chalk.red('Error:'), error instanceof Error ? error.message : 'Unknown error');
      if (process.env.DEBUG) {
        console.error(error);
...
`)
    ...
    if (showHelp) {
      console.log(`
${chalk.bold('happy')} - ${chalk.cyan('Happy Coder for Cursor')} - Control Cursor Agent from Anywhere

${chalk.bold('Usage:')}
  happy cursor            ${chalk.green('Start Cursor Agent with mobile/web control')}
  happy [options]         Start Claude with mobile control (legacy)
...
${chalk.bold.cyan('Cursor Agent Examples:')}
  happy cursor                     Start Cursor Agent session
  happy cursor --yolo              Start with auto-approve permissions (--force)
  happy cursor --mode plan         Start in plan/read-only mode
  happy cursor --mode ask          Start in ask/Q&A mode
  happy cursor --model gpt-5       Use a specific model
  happy cursor --resume <id>       Resume a previous session
  happy cursor --continue          Continue the last session
  happy cursor --sandbox enabled   Run in sandbox mode

${chalk.bold('Legacy Claude/Codex Examples:')}
...
${chalk.bold.cyan('Cursor Agent Options (from `agent --help`):')}
`)

      try {
        const { getCursorAgentPath } = await import('@/cursor/utils/cursorPath')
        const agentHelp = execFileSync(getCursorAgentPath(), ['--help'], { encoding: 'utf8' })
        console.log(agentHelp)
      } catch (e) {
        console.log(chalk.yellow('Could not retrieve Cursor Agent help. Make sure agent CLI is installed.'))
        console.log(chalk.gray('Install: https://docs.cursor.com/agent'))
      }
