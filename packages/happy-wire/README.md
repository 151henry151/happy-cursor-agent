...
`@happy-cursor/wire` centralizes definitions for:
...
In this repository, consumer workspaces import `@happy-cursor/wire` through package exports that point at `dist/*`.

That means on a clean checkout:
1. Build wire first: `yarn workspace @happy-cursor/wire build`
...
yarn workspace @happy-cursor/wire build
yarn workspace @happy-cursor/wire test
```
